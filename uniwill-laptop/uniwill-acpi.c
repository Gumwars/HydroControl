...
/*
 * Some devices expose both the charge threshold and the charging profiles.
 * Since only a single extension can be registered per battery, we have to
 * combine both property sets into a single extension for those devices.
 */
static const enum power_supply_property uniwill_charge_limit_modes_properties[] = {
	POWER_SUPPLY_PROP_CHARGE_TYPES,
	POWER_SUPPLY_PROP_HEALTH,
	POWER_SUPPLY_PROP_CHARGE_CONTROL_END_THRESHOLD,
};

static const struct power_supply_ext uniwill_charge_limit_modes_extension = {
	.name = DRIVER_NAME,
	.charge_types = BIT(POWER_SUPPLY_CHARGE_TYPE_TRICKLE) |
			BIT(POWER_SUPPLY_CHARGE_TYPE_STANDARD) |
			BIT(POWER_SUPPLY_CHARGE_TYPE_LONGLIFE),
	.properties = uniwill_charge_limit_modes_properties,
	.num_properties = ARRAY_SIZE(uniwill_charge_limit_modes_properties),
	.get_property = uniwill_get_property,
	.set_property = uniwill_set_property,
	.property_is_writeable = uniwill_property_is_writeable,
};
...
{
	bool limit = uniwill_device_supports(data, UNIWILL_FEATURE_BATTERY_CHARGE_LIMIT);
	bool modes = uniwill_device_supports(data, UNIWILL_FEATURE_BATTERY_CHARGE_MODES);

	if (limit && modes)
		return &uniwill_charge_limit_modes_extension;
...
static struct uniwill_device_descriptor hydroc16g1_descriptor __initdata = {
	.features = UNIWILL_FEATURE_FN_LOCK |
		    UNIWILL_FEATURE_SUPER_KEY |
		    UNIWILL_FEATURE_TOUCHPAD_TOGGLE |
		    UNIWILL_FEATURE_BATTERY_CHARGE_LIMIT |
		    ...
		    UNIWILL_FEATURE_USB_POWERSHARE,
	/*
	 * The chin bar on this model is driven entirely by an ITE 8233 USB
...
static const struct dmi_system_id uniwill_dmi_table[] __initconst = {
	{
		.ident = "Eluktronics HYDROC-16 G1",
		.matches = {
			DMI_MATCH(DMI_SYS_VENDOR, "ELUKTRONICS"),
			DMI_EXACT_MATCH(DMI_BOARD_NAME, "HYDROC-16 powered by premamod.com"),
		},
		.driver_data = &hydroc16g1_descriptor,
	},
	{
